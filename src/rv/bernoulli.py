...
class Bernoulli(RandomVariable):
    ...
    def _bayes(self, X):
        ...
        assert X.size == n_ones.sum() + n_zeros.sum(), (
    ...
    def _draw(self, sample_size=1):
        if isinstance(self.mu, np.ndarray):
            ...
            return (
                self.mu.n_ones / (self.mu.n_ones+self.mu.n_zeros) > np.random.uniform(size=(sample_size,)+self.shape)
            ).astype(np.int)
        elif isinstance(self.mu, RandomVariable):
            return (
                self.mu.draw(sample_size) > np.random.uniform(size=(sample_size,)+self.shape)
            )
